...
weekday_int <- filter(complete_activity, week=="weekday") %>%
                group_by(interval) %>% 
                summarize(mean=mean(steps,na.rm = TRUE))
with(weekday_int, plot(interval,mean, type="b", main="Average steps by 5-min daily intervals"), 
     ylab="Average steps", xlab="5-min time interval")
abline(h = mean(weekday_int$mean), col=2)

#Weekend points
weekend_int <- filter(complete_activity, week=="weekend") %>%
                group_by(interval) %>% 
                summarize(mean=mean(steps,na.rm = TRUE))
with(weekend_int, points(interval,mean, type="b", col=3)
...
weekday_int <- filter(complete_activity, week=="weekday") %>%
                group_by(interval) %>% 
                summarize(mean=mean(steps,na.rm = TRUE))
with(weekday_int, plot(interval,mean, type="b",  pch=".", col=1, main="Average steps by 5-min daily intervals"),
     ylab="Average steps", xlab="5-min time interval")
...
weekend_end <- filter(complete_activity, week=="weekend") %>%
                group_by(interval) %>% 
                summarize(mean=mean(steps,na.rm = TRUE))
with(weekend_end, points(interval,mean, type="b", pch=".", col=3))
